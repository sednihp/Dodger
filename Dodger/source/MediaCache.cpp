...
MediaCache::MediaCache() :	mScrWidth(600), mScrHeight(600),
							textColor({0,255,255}),
							window(nullptr), ren(nullptr), 
							imgCache(ren), fontCache(ren), txtCache(ren)
{
	window = SDL_CreateWindow("Dodger", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, mScrWidth, mScrHeight, SDL_WINDOW_SHOWN);
	if(!window)
	{
...
SDL_Color MediaCache::getTextColor()
{
	return textColor;
}